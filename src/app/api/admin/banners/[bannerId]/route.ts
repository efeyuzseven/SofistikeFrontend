...
async function proxyMutation(
  request: NextRequest,
  context: RouteContext<"/api/admin/banners/[bannerId]">,
  method: "PUT" | "DELETE",
) {
  ...
  try {
    const backendResponse = await fetch(
      `${API_BASE_URL}/api/v1/admin/banners/${encodeURIComponent(bannerId)}`,
      {
        method,
        headers: {
    ...
    return NextResponse.json(
      { message: "Banner yönetim servisine ulaşılamıyor." },
      { status: 503 },
    );
  }
}
